...
struct HTMLParsingTests {
    ...
    func basicDocumentParsing() throws {
    ...
    func recursiveTextExtraction() throws {
        let doc = try parseHTML(string: Self.sampleHTML)
    ...
    func uRLAndAttributeExtraction() throws {
        let doc = try parseHTML(string: Self.sampleHTML)
    ...
    func findThrowsOnExceedingMaxDepth() throws {
        let doc = try parseHTML(string: Self.sampleHTML)
...
extension XMLError: Equatable {
    public static func == (lhs: XMLError, rhs: XMLError) -> Bool {
        switch (lhs, rhs) {
        case let (.depthLimitExceeded(l), .depthLimitExceeded(r)):
            return l == r
        case let (.css(l), .css(r)):
            return l == r
        default:
            return false
        }
